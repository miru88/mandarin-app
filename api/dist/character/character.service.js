...
let CharacterService = class CharacterService {
    ...
    async findAll() {
        const chineseCharacters = await this.characterRepository.find({ order: { chinese: 'ASC' } });
        if (!chineseCharacters)
            return [];
        return chineseCharacters;
    }
    async findOne(characterId) {
        const chineseCharacter = await this.characterRepository.findOneBy({ id: characterId });
        if (!chineseCharacter)
            return null;
